...
export default function GrayscalePanel() {
  ...
  const [graysclaedImage, setGrayscaledImage] = useState(null);
  ...
  const handleReset = () => {
    setGrayscaledImage(null); // Reset to original image
  ...
  const handleApplyGrayscale = () => {
    if (uploadedImage) {
      ...
      image.onload = () => {
        ...
        // Create a new histogram array for grayscale
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          const gray = Math.round(0.299 * r + 0.587 * g + 0.144 * b);

          // Adjust grayscale value based on the slider
          const adjustedGray = Math.min(Math.max(gray + sliderValue, 0), 255); // Ensure it's within [0, 255]

          // Update pixel values to adjusted grayscale
          data[i] = adjustedGray; // Red
          data[i + 1] = adjustedGray; // Green
          data[i + 2] = adjustedGray; // Blue

          // Increment histogram for the adjusted grayscale value
          newHistogram[adjustedGray]++;
        }

        // Update the canvas with the grayscale image
        ctx.putImageData(imageData, 0, 0);
        setGrayscaledImage(canvas.toDataURL());
              ...
              {isUploaded && (
                <div className="flex justify-between w-full h-full mb-2">
                  ...
                  <div className="w-1/2 pl-2">
                    ...
                    {graysclaedImage ? (
                      <img
                        src={graysclaedImage} classNames
                        alt="Uploaded"
                        className="h-[88vh] w-full object-contain mx-auto rounded-xl border-2 border-gray-600" // Menambahkan border
                      />
                    ) : (
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (graysclaedImage) {
                            ...
                            link.href = graysclaedImage; // URL atau base64 image dari hasil edit
