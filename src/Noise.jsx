...
export default function ThresholdPanel() {
  ...
  const handleApplySaltAndPepperNoise = () => {
    if (uploadedImage) {
      const image = new Image();
      ...
      image.onload = () => {
        ...
        // Noise intensity controlled by slider (0 to 100)
        const noiseIntensity = Math.max(0, Math.min(sliderValue, 100)) / 100; // Convert to 0-1 range
  
        const addNoise = () => {
          for (let i = 0; i < data.length; i += 4) {
            const rand = Math.random(); // Random number between 0 and 1
  
            if (rand < noiseIntensity / 2) {
              // Add pepper (black pixel)
              data[i] = 0;         // Red
              data[i + 1] = 0;     // Green
              data[i + 2] = 0;     // Blue
            } else if (rand < noiseIntensity) {
        ...
        // Put the noisy image data back to the canvas
        ctx.putImageData(imageData, 0, 0);
        setThresholdedImage(canvas.toDataURL()); // Set the noisy image as the processed image
  
        // Update histogram data for each color channel (no change in logic)
        const redHistogram = Array(256).fill(0);
        const greenHistogram = Array(256).fill(0);
        const blueHistogram = Array(256).fill(0);
        ...
        <div className="flex flex-col w-full h-full min-h-screen p-4">
          <div className="flex flex-col flex-grow md:flex-row md:space-x-4 w-full">
            ...
            <div className="bg-gray-800 rounded-lg p-4 md:w-1/3">
              ...
              <button
                onClick={handleApplySaltAndPepperNoise}
