  ...
  const handleApplyBlur = () => {
    if (uploadedImage) {
      ...
      image.onload = () => {
        ...
        const blurIntensity = Math.max(1, Math.min(sliderValue, 25));
        const filterSize = Math.floor(blurIntensity / 10) * 2 + 1;
        ...
        // Use a sliding window approach for median filter
        const processBlock = (x, y, offset) => {
          const rPixels = [];
          const gPixels = [];
          const bPixels = [];
          ...
          for (let ky = -offset; ky <= offset; ky++) {
            for (let kx = -offset; kx <= offset; kx++) {
              ...
              rPixels.push(data[idx]);
              gPixels.push(data[idx + 1]);
              bPixels.push(data[idx + 2]);
            }
          }
  
          const medianR = median(rPixels);
          const medianG = median(gPixels);
          const medianB = median(bPixels);
  
          const idx = (y * canvas.width + x) * 4;
          newData[idx] = medianR;
          newData[idx + 1] = medianG;
          newData[idx + 2] = medianB;
  
          // Update histograms for each color channel
          redHistogram[medianR]++;
          greenHistogram[medianG]++;
          blueHistogram[medianB]++;
        };
  
        // Apply the median filter and calculate histograms
        for (let y = offset; y < canvas.height - offset; y++) {
          for (let x = offset; x < canvas.width - offset; x++) {
            processBlock(x, y, offset);
          }
        }
