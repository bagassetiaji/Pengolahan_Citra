  ...
  const handleSmoothImage = () => {
    if (uploadedImage) {
      ...
      image.onload = () => {
        ...
        // Ukuran kernel smoothing (3x3, 5x5, dll.)
        const kernelSize = 3;
        const kernelHalf = Math.floor(kernelSize / 2);
        const width = canvas.width;
        const height = canvas.height;
  
        // Membuat salinan data asli untuk proses smoothing
        ...
        // Mean smoothing dengan kernelSize x kernelSize
          ...
          for (let x = kernelHalf; x < width - kernelHalf; x++) {
            ...
            for (let ky = -kernelHalf; ky <= kernelHalf; ky++) {
              for (let kx = -kernelHalf; kx <= kernelHalf; kx++) {
                ...
                sumR += originalData[pos];
                sumG += originalData[pos + 1];
                sumB += originalData[pos + 2];
              }
            }
  
            // Hitung rata-rata untuk setiap channel warna
            const numPixels = kernelSize * kernelSize;
            ...
            data[idx] = avgR;     // Red
            data[idx + 1] = avgG; // Green
            data[idx + 2] = avgB; // Blue
        ...
        // Update the canvas with the smoothed image
