...
export default function NegativePanel() {
  ...
  const handleReset = () => {
    setNegativeImage(null); // Reset to original image
  ...
  const handleCompressImage = (compressionLevel) => {
    if (uploadedImage) {
      const image = new Image();
      ...
      image.onload = () => {
        ...
        // Set the compression size based on the selected level
        let compressionFactor;
        if (compressionLevel === "small") {
          compressionFactor = 0.1; // 10% of the original size
        } else if (compressionLevel === "medium") {
          compressionFactor = 0.3; // 30% of the original size
        } else if (compressionLevel === "high") {
          compressionFactor = 0.5; // 50% of the original size
        }
  
        // Calculate the new dimensions for the compressed image
        const newWidth = image.width * compressionFactor;
        const newHeight = image.height * compressionFactor;
        ...
        // Create histograms for each color channel (Red, Green, Blue)
        const redHistogram = Array(256).fill(0);
        const greenHistogram = Array(256).fill(0);
        const blueHistogram = Array(256).fill(0);
  
        // Calculate the histogram for the resized image by iterating over each pixel
        for (let i = 0; i < data.length; i += 4) {
          const red = data[i];     // Red channel
          const green = data[i + 1]; // Green channel
          const blue = data[i + 2]; // Blue channel
  
          // Increment the histogram for each color channel
          redHistogram[red]++;
        ...
        setNegativeImage(canvas.toDataURL()); // Set the compressed image
  ...
  const handleSmallCompression = () => handleCompressImage("small");
  
  // For medium compression
  const handleMediumCompression = () => handleCompressImage("medium");
  
  // For high compression
  const handleHighCompression = () => handleCompressImage("high");
  ...
  {/* Tombol Apply Negative diubah menjadi 3 kolom */}
  <div className="flex space-x-4 mb-4">
    <button
      onClick={handleSmallCompression}
      className="bg-slate-500 hover:bg-slate-600 text-white px-4 py-2 rounded-md w-1/3"
    >
      High Compres
    </button>
    <button
      onClick={handleMediumCompression}
      className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md w-1/3"
    >
      Medium Compres
    </button>
    <button
      onClick={handleHighCompression}
      className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-md w-1/3"
    >
      Low Compres
    </button>
  </div>
  ...
  <button
    onClick={handleReset}
    className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md mb-6 w-full"
  >
    Reset
  </button>
